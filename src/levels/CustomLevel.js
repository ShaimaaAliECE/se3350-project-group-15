...
export default function CustomLevel() {
  ...
  const blurArrayValues = (val) => {
    ...
    const reg = /[(\[)(\])(\【)(\】)(\，)(\。)(\ )(\~)(\!)(\@)(\#) (\$)(\%)(\^)(\&)(\*)(\-)(\_)(\+)(\=)(\{)(\})(\|)(\\)(\;)(\:)(\')(\")(\,)(\.)(\/) (\)(\?)(\)]+/;
    const tempArr = tempArrVals.split(reg).filter(ele => !!ele && !isNaN(Number(ele))).map(item => Number(item));
    const exceedFlag = tempArr.some(element => element > 999);
    if (tempArr.length !== 0 && (exceedFlag || tempArr.length < 5 || tempArr.length > 8)) {
        ...
        <div className='custom-level-inputs-box'>
          ...
          <div className='custom-level-inputs-form'>
            <form className='w-100'>
              <label className='w-100 d-flex align-items-center justify-content-center' style={{marginBottom: "16px"}}>
                <Col sm={3} className="text-align-left">Array size:</Col>
                <Col sm={8} className="text-align-left">
                  <select className='w-50' style={{ border:"1px solid #e5e7e8",height:"38px",borderRadius:"3px"}} value={curentArraySize} onChange={selectArraySize}>
              ...
              <label className='w-100 d-flex align-items-center justify-content-center'>
                <Col sm={3} className="text-align-left">Array Values (optional):</Col>
                <Col sm={8}>
                  <input type="text" value={currentArrayValuesStr} onChange={changeArrayValues} onBlur={blurArrayValues} className='w-100' style={{ border:"1px solid #e5e7e8",height:"38px",borderRadius:"3px"}} />
                </Col>
              </label>
            </form>
            <div className='form-group custom-level-form' style={{marginTop: "16px"}}>
              <label class="col-sm-3"></label>
              <div class="col-sm-8 text-align-left">
                <Button style={{marginRight: "8px"}} onClick={levelStart} disabled={runDisabled}>Run</Button>
                <Button onClick={levelReset} class="btn btn-default" style={{color: "#0d6efd",backgroundColor: "#FFFFFF"}}>Reset</Button>
              </div>
            </div>
          </div>
